...
export const instanceSchema = yup.object({
  ...
  attributes: yup
    ...
    .test("all-attribute-validation", "", function () {
      const findRequiredAttributeIndex = this.options.context?.[
        "attributes"
      ]?.findIndex((attribute: InstanceMetaDataField) => attribute?.isRequired);
      ...
      return this.createError({
        path: "attributes",
        message: "Please set values for all required attributes",
      });
    }),
  metrics: yup
    .array()
    ...
    .test("all-metric-validation", "", function () {
      if (!this.parent?.metrics) {
        return this.createError({
          path: "metrics",
          message: "Please set metric behavious for all metrics",
        });
      }

      if (
        this.parent?.metrics?.findIndex(
          (metric: InstanceMetricForm) =>
            metric?.metricBehaviour && metric?.metricBehaviour !== ""
        ) !== -1
      ) {
        return true;
      }
